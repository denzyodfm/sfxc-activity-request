          ...
          {attachments.length === 0 ? (
            ...
            attachments.map((attachment) => (
              <div key={attachment.id} className="rounded-3xl border border-slate-200 p-4">
                ...
                <a href={`/api/attachments/${attachment.id}`} target="_blank" rel="noreferrer" className="mt-2 inline-flex rounded-2xl bg-slate-100 px-3 py-2 text-sm text-slate-700 hover:bg-slate-200">
                  Open attachment
                </a>
              </div>
            ))
