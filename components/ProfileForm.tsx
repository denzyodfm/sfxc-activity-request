...
export default function ProfileForm({ user }: ProfileFormProps) {
  ...
  return (
    <form className="sfxc-card max-w-4xl space-y-6 p-6" onSubmit={handleSubmit}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-slate-50">
          {user.profilePictureUrl || profilePicture ? (
            ...
            <span className="text-sm font-semibold text-slate-500">Photo</span>
          )}
        </div>
        <label className="block text-sm text-slate-700">
          ...
          />
          <span className="mt-1 block text-xs text-slate-500">Maximum file size: 1MB.</span>
        </label>
      </div>
